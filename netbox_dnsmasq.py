...
def check_duplicates(nb_dhcp: dict, nb_dns: dict, enable_duplicates: bool) -> tuple[dict, dict]:
    """
    ...
    if len(dhcp_macs) != len(set(dhcp_macs)) or len(dhcp_ips) != len(set(dhcp_ips)):
        logger.warning("Duplicate MAC or IP addresses found. Removing from DHCP config.")
        for k, v in nb_dhcp.copy().items():
            if dhcp_macs.count(v['mac']) > 1 or dhcp_ips.count(k) > 1:
                logger.info(f"Duplicate MAC found: {v['mac']} - {k} - removing from DHCP config.")
                del dhcp_macs[dhcp_macs.index(v['mac'])]
                del dhcp_ips[dhcp_ips.index(k)]
                del nb_dhcp[k]
            if len(dhcp_macs) == len(set(dhcp_macs)) and len(dhcp_ips) == len(set(dhcp_ips)):
                break
    dns_ips = []
    for fqdn in nb_dns:
        ...
        for k, v in nb_dns.copy().items():
            if dns_ips.count(v['ip']) > 1:
                if not enable_duplicates:
                    logger.info(f"Duplicate IP address {v['ip']} found. Removing from DNS config.")
                    del dns_ips[dns_ips.index(v['ip'])]
                    del nb_dns[k]
                else:
                    if k in nb_dns:
                        logger.info(f"Duplicate IP address {v['ip']} found. "
                                    f"Related FQDN's will be merged into one entry.")
...
def get_ip_data(dhcp_ignore_tag: str, dhcp_tag: str, nb: pynetbox.api) -> tuple[dict, dict]:
    """
    ...
    for ip in ip_addresses:
        ...
        if len(ip.dns_name) > 0:
            nb_dns.update({ip.dns_name: {'tags': [], 'ip': ''}})
        else:
            logger.warning(f"{ip_address} has {dhcp_ignore_tag} tag, but no FQDN.")
            continue
        for tag in ip.tags:
            if ip.dns_name:
                nb_dns[ip.dns_name]['tags'].append(str(tag))
        if dhcp_tag in list(t.name for t in ip.tags):
            logger.warning(f"{ip} has {dhcp_tag} tag, but also has {dhcp_ignore_tag} tag.")
        if len(ip.dns_name) > 0:
            if ip.dns_name:
            ...
            if ip.assigned_object_id:
                if ip.assigned_object.mac_address is not None and dhcp_ignore_tag not in list(t.name for t in ip.tags):
                    ...
                    if ip_address in nb_dhcp:
                        logger.warning(f"Duplicate IP address {ip_address}"
...
def write_config(dhcplist: list, dnslist: list, dns_loc: str | pathlib.Path, dhcp_loc: str | pathlib.Path) -> None:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('-e', '--enable-duplicate-ips', action='store_true',
    ...
    check_duplicates(nb_dhcp=dhcp, nb_dns=dns, enable_duplicates=args.enable_duplicate_ips)
    dhcp_data = format_dhcp(dhcp)
    dns_data = format_dns(dns)
    write_config(dhcplist=dhcp_data, dnslist=dns_data, dns_loc=DNS_hosts_location, dhcp_loc=DHCP_config_location)
    if not args.dev:
        restart_service()
